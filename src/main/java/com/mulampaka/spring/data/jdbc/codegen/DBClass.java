...
//        }
     ...
     *将主键转为map
     */
    protected void printPkeyUnMapper() {
        ...
        if (this.pkeys.size () == 1)
        {
            ...
            // create unmapper
            sourceBuf.append("\tpublic static final RowUnmapper<" + keyType.getName() + "> ROW_UNMAPPER = new " + name + "PkeyUnmapper ();\n");
            sourceBuf.append("\tpublic static final class " + name + "PkeyUnmapper implements RowUnmapper<" + keyType.getName() + ">\n");
            this.printOpenBrace(1, 1);
            String objName = name.toLowerCase();
            sourceBuf.append("\t\tpublic Map<String, Object> mapColumns(" + keyType.getName() + " " + objName + ")\n");
            this.printOpenBrace(2, 1);
            sourceBuf.append("\t\t\tMap<String, Object> mapping = new LinkedHashMap<String, Object>();\n");
            sourceBuf.append("\t\t\tmapping.put(COLUMNS." + key.toUpperCase() + ".getColumnName(), " +  objName + ");\n");
